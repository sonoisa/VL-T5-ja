        ...
        **kwargs,
        ...
        if vis_extra_ids > 0:
            additional_special_tokens.extend(
                [f"<vis_extra_id_{i}>" for i in range(vis_extra_ids)]
            )
        ...
        if index < self.sp_model.get_piece_size():
            ...
            if index > self.sp_model.get_piece_size() + self._extra_ids - 1:
                ...
                token = "<extra_id_{}>".format(
                    self.vocab_size - self._vis_extra_ids - 1 - index
                )
        return token
...
class VLT5Converter(SpmConverter):
    def vocab(self, proto):
        ...
        vocab += [
            ("<extra_id_{}>".format(i), 0.0) for i in range(num_extra_ids - 1, -1, -1)
        ]

        num_vis_extra_ids = self.original_tokenizer._vis_extra_ids
        vocab += [
            ("<vis_extra_id_{}>".format(i), 0.0)
