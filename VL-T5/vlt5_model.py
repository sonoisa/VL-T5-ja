...
class JointEncoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        assert self.is_decoder is False

        self.visual_embedding = VisualEmbedding(config, embed_tokens)
    ...
    def parallelize(self, device_map=None):
        ...
        # Set visual_embedding to first layer
        self.visual_embedding = self.visual_embedding.to(self.first_device)
    ...
    def deparallelize(self):
        ...
        self.visual_embedding = self.visual_embedding.to("cpu")
    ...
    def forward(
        ...
        if vis_attention_mask is None:
            vis_attention_mask = attention_mask.new_ones(batch_size, V_L)
        ...
        extended_attention_mask = self.get_extended_attention_mask(
            attention_mask, (batch_size, seq_length + V_L), inputs_embeds.device
        )
        ...
        for i, (layer_module, past_key_value) in enumerate(
            ...
            if getattr(self.config, "gradient_checkpointing", False) and self.training:
                ...
                layer_outputs = checkpoint(
                    ...
                    past_key_value=None,  # past_key_value is always None with gradient checkpointing
                )
            else:
        ...
        **kwargs,
        ...
        if self.model_parallel:
            ...
            if vis_attention_mask is not None:
                vis_attention_mask = vis_attention_mask.to(self.decoder.first_device)
...
class VLT5ModelWrapper(VLT5Model):
    def __init__(self, config):
        super().__init__(config)
    ...
    def forward(
        ...
        **kwargs,
        ...
        for _loss, task in zip(loss.detach(), task):
...
def create_config(backbone, args):
